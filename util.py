...
from models import Comments, Followers, Likes, Post, UploadedBy, User
...
def getUser(username):
    ...
    if user.profile_picture and type(user.profile_picture)!=type('somestring'):
        user.profile_picture = base64.b64encode(user.profile_picture).decode("utf-8")
    return user

def getPosts(username, forHomePage = False):
    post_likes = {}
    post_comments = {}
    isLiked = {}
    user = {}
    ...
    if forHomePage:
        return posts, post_likes, post_comments, isLiked, user
    return posts, post_likes, post_comments, isLiked
